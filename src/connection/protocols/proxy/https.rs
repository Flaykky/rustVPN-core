...
impl Protocol for HttpsProxy {
    ...
    async fn connect(&self) -> Result<Self::Connection, VpnError> {
        // First establish HTTP CONNECT tunnel
        ...
        // Upgrade to TLS here (simplified - actual implementation would use rustls/tokio-rustls)
...
impl Connection for HttpsProxyConnection {
    async fn send_packet(&mut self, packet: &[u8]) -> Result<(), VpnError> {
        self.stream.write_all(packet).await
            .map_err(|e| VpnError::ConnectionError(format!("Не удалось отправить данные через HTTPS-прокси: {}", e)))
    }

    async fn receive_packet(&mut self) -> Result<Vec<u8>, VpnError> {
        let mut buffer = vec![0u8; 65535];
        let n = self.stream.read(&mut buffer).await
            .map_err(|e| VpnError::ConnectionError(format!("Не удалось получить данные через HTTPS-прокси: {}", e)))?;
        buffer.truncate(n);
        Ok(buffer)
    }
