...
/*
Основные функции:
- Определяет централизованную систему ошибок для VPN-ядра
- Предоставляет типизированные ошибки для различных компонентов системы
- Реализует автоматические конверсии из стандартных ошибок (std::io, serde)
- Включает конструкторы для удобного создания ошибок

Категории ошибок:
1. ConfigError - проблемы с конфигурацией
2. ConnectionError - сбои подключения
3. ProtocolError - ошибки протоколов связи
4. ObfuscationError - сбои обфускации трафика
5. EncryptionError - проблемы шифрования
6. TunnelingError - ошибки туннелирования
7. CliError - обработка командной строки
8. PluginError - загрузка/выполнение плагинов
9. Unknown - неуточненные ошибки

Примеры использования:
// Создание кастомной ошибки
let err = VpnError::config_error("Отсутствует конфигурационный файл");

// Конверсия из IO-ошибки
let io_result: Result<(), std::io::Error> = ...;
let vpn_result: Result<(), VpnError> = io_result.map_err(|e| e.into());
*/
